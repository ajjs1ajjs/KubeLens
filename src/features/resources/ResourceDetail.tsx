...
export function ResourceDetail({
  ...
  const [activeTab, setActiveTab] = useState(defaultTab ?? "logs");
  ...
  useEffect(() => {
    if ((activeTab === "logs" || activeTab === "metrics") && tabsAnchorRef.current && scrollContainerRef.current) {
  ...
  return (
    <div className="bg-background flex h-full flex-col overflow-hidden border-l">
      ...
      {/* Tabs — only Logs & Metrics, styled like YAML view/edit sheets */}
      {isPod && resourceCtx && (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col">
          ...
          <TabsContent value="logs" className="mt-3 data-[state=inactive]:hidden">
            <div className="min-h-[420px] rounded-md border bg-background">
              <LogsViewer
                ctx={resourceCtx}
                name={m.name}
