...
function DetailRow({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[150px_minmax(0,1fr)] gap-3 py-1">
      ...
      <span className="min-w-0 overflow-hidden text-xs break-all whitespace-pre-wrap [overflow-wrap:anywhere]">
        {children}
      </span>
    </div>
  );
...
function Section({
  ...
  return (
    <div className="mt-1.5 border-t pt-1.5">
      ...
      {open && <div className="mt-1 flex min-w-0 flex-col gap-0.5 overflow-hidden pl-4">{children}</div>}
    </div>
  );
}
