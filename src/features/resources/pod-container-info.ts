...
function readProbe(raw: Record<string, unknown> | undefined): ContainerProbe | undefined {
  ...
  if (raw.httpGet && typeof raw.httpGet === "object") handler.httpGet = raw.httpGet as ProbeHandler["httpGet"];
  ...
  return {
    initialDelaySeconds: typeof raw.initialDelaySeconds === "number" ? raw.initialDelaySeconds : undefined,
    ...
    terminationGracePeriodSeconds:
      typeof raw.terminationGracePeriodSeconds === "number" ? raw.terminationGracePeriodSeconds : undefined,
    handler,
  };
}

function readSecurityContext(raw: Record<string, unknown> | undefined): SecurityContext | undefined {
  if (!raw || typeof raw !== "object") return undefined;
  ...
  if (typeof raw.readOnlyRootFilesystem === "boolean") ctx.readOnlyRootFilesystem = raw.readOnlyRootFilesystem;
  if (typeof raw.allowPrivilegeEscalation === "boolean") ctx.allowPrivilegeEscalation = raw.allowPrivilegeEscalation;
...
export function podContainerInfo(pod: K8sObject): ContainerInfo[] {
  ...
  return containers.map((container): ContainerInfo => {
    ...
    return {
      ...
      memoryRequest: reqBlock ? parseQuantity(reqBlock.memory as string | number, "memory") : undefined,
      memoryLimit: limBlock ? parseQuantity(limBlock.memory as string | number, "memory") : undefined,
      ports,
      env,
      volumeMounts,
      ...
      securityContext: readSecurityContext(container.securityContext as Record<string, unknown> | undefined),
